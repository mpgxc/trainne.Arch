...
class CreateSessionService {
    constructor(
        private CreateSessionCookieService: CreateSessionCookieService,
    ) {}

    async execute({
        password,
        username,
    }: ISessionRequest): Promise<ISessionDTO> {
        ...
        try {
            ...
            if (
                response.status !== 200 &&
                response.request.path !== '/assignments'
            ) {
            ...
            return { username: username, sessionCookie: CookieValue.value };
        } catch (error) {
